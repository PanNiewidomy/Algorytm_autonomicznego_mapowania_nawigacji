...
def solve_kidnap(orig_scan_img, map_image, min_distance, map_resolution = 0.05, max_iterations = 30, stop_search_threshold = 50, lidar_range = 8.0, map_origin = [0,0,0], threshold_distance = 0.15):
    ...
    candidate_area = dt.get_distance_transform(map_image, distance, map_resolution = map_resolution, threshold_px = threshold_distance/map_resolution) #Rgb image, which red area where the robot can lie only.
    ...
    for _ in range(max_iterations):
        ...
        if num_red_pixels > 0:
            ...
            cv2.circle(candidate_area, (y, x), 10, (0, 255, 0), -1) # Draw GREEN color, so next random selection should not occur from here
            red_pixels = np.where(candidate_area[:,:,0] == 255)  # Find the indices of red pixels
            num_red_pixels = len(red_pixels[0])  # Get the number of red pixels
            print(f"Number of red pixels left: {num_red_pixels}")

    print(f"Random coords: {random_coords}")
    ...
    map_origin = np.array(map_origin[:2]) # Find the origin values from map .yaml file.
    ...
    yaw = best_theta_degrees * math.pi / 180
    print("----------------------------")
    print("Robot in map meters: ", robot_in_map_meters)
    print("Robot theta (on map) in degrees: ", math.degrees(best_theta_degrees))
    print("Robot yaw (on map) in radians: ", yaw)
    print("----------------------------")
    ...
    end_x = int(robot_on_map[0] + vector_length * math.cos(yaw))
    end_y = int(robot_on_map[1] + vector_length * math.sin(yaw))
    ...
    return robot_in_map_meters, yaw